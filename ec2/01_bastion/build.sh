...
# ssh
aws ec2 authorize-security-group-ingress --group-id "${sg_id}" --protocol 'tcp' --port 22 --cidr ${BASTION_FROM_ONLY}
...
ssh:remote_exec_shell_script \
       ...
       '../../scripts/aws/ec2_init/ec2_init_for_bastion.sh' \
       /tmp/ \
       "sudo /tmp/ec2_init_for_bastion.sh \
